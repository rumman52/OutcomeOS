...
Initial namespaced types are `lead.captured`, `lead.qualified`, `lead.accepted`, `order.created`, `order.confirmed`, `fulfillment.delivered`, `order.returned`, `booking.created`, `booking.attended`, `booking.no_show`, `account.activated`, `payment.succeeded`, `payment.refunded`, `outcome.verified`, `outcome.billable`, `dispute.opened`, `dispute.resolved`, and `invoice.paid`. `lead.accepted` is the explicit provider-neutral business-acceptance fact; qualification, attribution metadata, and free-form payload text cannot substitute for it.
...
The deterministic evaluator consumes sanitized canonical projections. The planned durable
`evaluate.outcome.v1` job and its bounded operational events are internal contracts, not evidence
of a live provider or billing integration.
