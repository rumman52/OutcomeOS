...
from outcomeos_api.models import (
    BillableResult, Contact, Conversation, DisputeEvidence, LedgerEntry, Order, Outcome,
)


def test_external_records_have_tenant_scoped_idempotency_constraints():
    for model in (Contact, Conversation, Order, Outcome, DisputeEvidence, BillableResult, LedgerEntry):
        constraints = {
...
def test_high_risk_tables_are_forced_through_rls():
    migration = (Path(__file__).parents[1] / "migrations/versions/20260804_0001_mvp_tenant_schema.py").read_text()
    for table in (
