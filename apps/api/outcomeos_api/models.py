...
class Contact(Base, IdMixin, TenantOwned, ExternalMixin):
    ...
    __table_args__ = (UniqueConstraint("tenant_id", "provider", "external_id", name="uq_contacts_external"),)
...
class Conversation(Base, IdMixin, TenantOwned, ExternalMixin):
    ...
    __table_args__ = (UniqueConstraint("tenant_id", "provider", "external_id", name="uq_conversations_external"),)
...
class KnowledgeDocument(Base, IdMixin, TenantOwned, ExternalMixin):
    ...
    __table_args__ = (UniqueConstraint("tenant_id", "provider", "external_id", name="uq_knowledge_documents_external"),)
    title: Mapped[str] = mapped_column(String(500), nullable=False)


class KnowledgeChunk(Base, IdMixin, TenantOwned):
    __tablename__ = "knowledge_chunks"
    __table_args__ = (UniqueConstraint("tenant_id", "document_id", "chunk_index", name="uq_knowledge_chunks_position"),)
    document_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("knowledge_documents.id"), nullable=False)
    chunk_index: Mapped[int] = mapped_column(Integer, nullable=False)
    content: Mapped[str] = mapped_column(Text, nullable=False)
    # pgvector deployments replace this portable representation with vector(n).
    embedding: Mapped[str | None] = mapped_column(Text)


class Order(Base, IdMixin, TenantOwned, ExternalMixin, MoneyMixin):
    __tablename__ = "orders"
    __table_args__ = (UniqueConstraint("tenant_id", "provider", "external_id", name="uq_orders_external"),)
    contact_id: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("contacts.id"))
...
class Outcome(Base, IdMixin, TenantOwned, ExternalMixin):
    ...
    __table_args__ = (UniqueConstraint("tenant_id", "provider", "external_id", name="uq_outcomes_external"),)
...
class DisputeEvidence(Base, IdMixin, TenantOwned, ExternalMixin):
    ...
    __table_args__ = (UniqueConstraint("tenant_id", "provider", "external_id", name="uq_dispute_evidence_external"),)
...
class BillableResult(Base, IdMixin, TenantOwned, ExternalMixin, MoneyMixin):
    ...
    __table_args__ = (UniqueConstraint("tenant_id", "provider", "external_id", name="uq_billable_results_external"),)
